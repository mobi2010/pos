...
class SettingClient extends Model
{
     ...
     * @var string
     */
    protected $primaryKey = 'setting_client_id';
     ...
     * @var string
     */
    public $setting_clientCacheKey = "setting_client:";
    ...
    public function getOne($params=[]){
        ...
        if(isset($params[$this->primaryKey])) {
            ...
            $cacheKey = $this->setting_clientCacheKey . $id;
        ...
        if(empty($res)) {
            $row = self::where($params)->orderBy($this->primaryKey,'desc')->get()->toArray();
            if(!empty($row)) {
                $res = $row[0];
            }
        }
